...
def get_support_json_schema():
    """
    Read SUPPORT_JSON_SCHEMA environment variable and validate its value.
    
    Returns:
        bool: True if environment variable is set to 'true', False if set to 'false'
        
    Raises:
        ValueError: If environment variable is missing or has invalid value
    """
    env_value = os.getenv('SUPPORT_JSON_SCHEMA')
    
    if env_value is None:
        raise ValueError("Environment variable SUPPORT_JSON_SCHEMA is not set")
    ...
    if env_value_lower == 'true':
        ...
        raise ValueError(f"Invalid value for SUPPORT_JSON_SCHEMA environment variable: '{env_value}'. Must be 'true' or 'false'")
...
class BaseConfig:
    ...
    openie_mode: Literal["offline", "online", "edu_based_ee_online", "edu_based_contextual_ee_online"] = field(
        default="edu_based_ee_online",
    ...
    # Date format specific attributes
    date_format_type: Literal["locomo", "longmemeval"] = field(
        default="locomo",
        metadata={"help": "Type of date format used in the dataset. 'locomo' for LoCoMo format (e.g., '1:56 pm on 8 May, 2023'), 'longmemeval' for LongMemEval format (e.g., '2023/05/30 (Tue) 23:40')."}
    )
    
    query_to_session_retrieval: bool = field(
        default=False,
        metadata={"help": "Whether to have query to session for retrieval for conversation setting."}
    )
    skip_edu_context_gen: bool = field(
        default=True,
        metadata={"help": "Whether to skip EDU context generation during conversation indexing. If True, uses placeholder text instead of generating contexts."}
    )
    skip_retrieval_ppr: bool = field(
        default=False,
        metadata={"help": "Whether to skip PPR for retrieval."}
    )
